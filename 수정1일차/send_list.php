...
// 게시글
$main_sql = "SELECT to_char(m.m_datetime,'YYYY.MM.DD HH24:mi:ss') as m_datetime, m.m_title, m.wr_user, de.dept_name, du.duty_name, u.user_name, m.is_read, m.mno
FROM ex_user_set u, ex_dept_set de, board_message m, ex_duty_set du
WHERE m.re_user = u.uno and u.dept_id = de.dept_no and u.duty_id = du.duty_no and m.wr_user = {$row[1]["uno"]}
ORDER BY mno desc";
$sql = "WITH A as (
{$main_sql}
)
...
if ($query_row_count > 0) {
    ...
    while ($db->next_record()) {
        ...
        if ($row["is_read"] == "Y") {
            ...
            $str_data_row .= "              
<tr onclick=\"location.href='send_view.php?{$list_query}&mno={$row["mno"]}'\" style=\"cursor:pointer\">
    ...
    <td style=\"text-align: center;\">{$state}</td>
            ...
            $str_data_row .= "              
<tr onclick=\"location.href='send_view.php?{$list_query}&mno={$row["mno"]}'\" style=\"cursor:pointer\" style=\"cursor:pointer\">
    <td style=\"text-align: center;\">{$no}</td>
    <td>{$m_title}</td>
    <td>{$dept_user}</td>
    <td style=\"text-align: center;\">{$date}</td>
...
<body>
    ...
    <section>
        <div id="main_content">
            <div id="message_box">
                ...
                <table class="table table-hover">
                    <thead>
                        <tr>
                            ...
                            <th scope="cols" style="width: 360px;">받는 이</th>
                ...
                <span><input type="button" value="수신 쪽지함" onclick="goMessage()" style="margin-left:75%; font-size:small; width:min-content"></span>
    ...
    <script>
        function goSend() {
            location.href = 'message_send.php';
        }
