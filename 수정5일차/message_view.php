...
// 게시글
$main_sql = "SELECT to_char(m.m_datetime,'YYYY.MM.DD HH24:mi:ss') as m_datetime, m.m_title, m.wr_user, de.dept_name, du.duty_name, u.user_name, m.is_read, m.m_body, m.re_user, u.is_use, u.is_active
FROM ex_user_set u, ex_dept_set de, board_message m, ex_duty_set du
WHERE m.wr_user = u.uno and u.dept_id = de.dept_no and u.duty_id = du.duty_no and m.re_user = {$row[1]["uno"]} and mno={$post["mno"]}";
...
if ($row[0]["is_use"] == 'N' || $row[0]["is_active"] == 'N') {
    ...
    $reply = "<span>
                <input type=\"button\" class=\"btn btn-default btn-lg btn-block\" value=\"답장보내기\" onclick=\"goSend()\" style=\"margin-left:260px; font-size:small; width:min-content\">
            </span>";
  }
