...
<body>
    <form id="findpwd_frm" name="findpwd_frm" action="findpwd_process.php" method="POST">
        <h1>비밀번호 찾기</h1>
        <div class="input-box">
            <input id="user_id" type="text" name="user_id" placeholder="아이디" value="<?php echo $user_id?>">
            <label for="user_id">아이디</label>
        </div>

        <div class="input-box">
            <input id="pwd_answer" type="text" name="pwd_answer" placeholder="자신의 고향을 적으세요">
            <label for="pwd_answer">자신의 고향을 두 글자로 입력하세요</label>
        </div>
...
<script>
    function null_chk() {
        ...
        var password = document.getElementById("pwd_answer").value;
        ...
        if (id == '') {
            ...
            document.getElementById("pwd_answer").focus();
        } else if (!(password.length == 2)) {
            alert("두 글자로 입력해주세요.");
            document.getElementById("pwd_answer").focus();  
        } else {
            document.getElementById("findpwd_frm").submit();
        }
    }
