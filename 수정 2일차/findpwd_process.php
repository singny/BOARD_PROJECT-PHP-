...
$db = new DB;
$sql = "SELECT * FROM {$_table_user} WHERE user_id = '{$post["user_id"]}' AND pwd_answer = '{$post["pwd_answer"]}'";
$db->query($sql);
$n = $db->nf();
$db->next_record();
@$row[0] = $db->Record;
...
if ($n == 0) {
    FUN::alert("해당하는 정보를 찾을 수 없습니다.", "find_pwd.php");
} else{
...
<body>
    <form id="pwd_frm" name="pwd_frm" action="board_user_modify.php?mode=pwd_modify" method="POST">
        ...
        <div class="input-box">
            ...
            <input type="password"  id="user_pwd" name="user_pwd"placeholder="4자리이상의 비밀번호를 입력하세요." value="" required>
            <label for="user_name">4자리이상의 비밀번호를 입력하세요.</label>
        </div>

        <div class="input-box">
            <!-- <input id="phone" type="text" name="phone" placeholder="핸드폰 번호"> -->
            <input type="password"  id="user_pwd_chk"  value="" required placeholder="비밀번호 확인">
            <label for="phone">비밀번호 확인</label>
        </div>
        <br />
        <input type="button" value="변경 완료" onclick="pwdModify()"><br /><br />
...
<script>
    ...
    function pwdModify() {
        ...
        if(pwd == ''){
            ...
            document.getElementById("pwd_frm").submit();
        }
    }
</script>
