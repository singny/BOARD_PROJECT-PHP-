...
if ($post["mode"] == "modify" && $post["con_no"] != "") {
    ...
    if (!$post["re_user"]) {
        ...
        if ($post["yn_img"] == "no") {
            ...
            if (move_uploaded_file($_FILES['userfile']['tmp_name'], $uploadfile)) {
                ...
                $file_sql = "UPDATE {$_table_board} SET FILE_PATH = '{$uploadfile}' WHERE CON_NO={$post["con_no"]}";
                $db->query($file_sql);
                $db->commit();
            } else {
    ...
    if (move_uploaded_file($_FILES['userfile']['tmp_name'], $uploadfile)) {
        //echo "파일이 유효하고, 성공적으로 업로드 되었습니다.\n";
        $file_sql = "UPDATE {$_table_board} SET FILE_PATH = '{$uploadfile}' WHERE CON_NO={$con_no}";
        $db->query($file_sql);
        $db->commit();
    } else {
        //print "파일 업로드 공격의 가능성이 있습니다!\n";
    ...
    $db->BeginTransaction();
    try {
        $ano = $db->nextid("SEQ_" . $_table_alert);
        $alert_sql = "INSERT INTO {$_table_alert} ("
            . " ANO, UNO, CON_NO, WR_USER, A_DATETIME"
            . ") VALUES ("
            . " {$ano},{$post["re_user"]},{$con_no},{$post["uno"]},sysdate"
            . ")";
        $db->query($alert_sql);
        $db->commit();
    } catch (Exception $ex) {
    ...
    try {
        ...
        $sql = "INSERT INTO {$_table_comment} ("
            ...
            . ") VALUES ("
            . " {$com_no},{$post["con_no"]}, sysdate ,'{$post["user_id"]}','{$com_body}','Y','{$post["scroll"]}'"
            . ")";
        $db->query($sql);
        $db->commit();
    } catch (Exception $ex) {
    ...
    $comment_sql = "UPDATE {$_table_board} set con_comment = con_comment+1 where con_no ={$post["con_no"]}";
