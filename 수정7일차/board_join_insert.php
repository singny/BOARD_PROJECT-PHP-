...
try{
    ...
    $sql = "INSERT INTO {$_table_user} ("
            . " UNO, USER_ID, USER_PWD, USER_NAME, DEPT_ID, TEMP_ID, DUTY_ID, LOC_ID, IS_ACTIVE, IS_USE, PHONE, PWD_ANSWER"
            . ") VALUES ("
            . " {$uno},'{$post["user_id"]}','{$post["user_pwd"]}', '{$post["user_name"]}','{$post["dept_id"]}','{$post["dept_id"]}','{$post["duty_id"]}','{$post["loc_id"]}','Y','Y','{$post["phone"]}','{$post["pwd_answer"]}'"
            . ")";
    $db->query($sql);
    $db->commit();
} catch (Exception $ex) {
